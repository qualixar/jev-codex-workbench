...
def validate_response(raw, questions, expected_model=None):
    ...
    for key,q in questions.items():
        ...
        if q['type']=='choice':
            ...
            v=a.get('score')
            if not number(v,0,len(labels)-1) or abs(v-sum(int(k)*x for k,x in p.items())) > .01: raise AutoError('SCORE_EXPECTATION')
...
def compact_receipt(record):
    ...
    out={'version':'1.1.3','case_id':record.get('case_id'),
