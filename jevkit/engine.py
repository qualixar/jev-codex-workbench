...
def run(case_id: str, variant: str = 'nominal', mode: str = 'fixture', state=None,
    ...
    s=spec(case_id,root);f=fixture(case_id,variant,root);custom=state is not None
    if mode not in ('fixture','live'):raise SafeError('INVALID_MODE')
    if mode=='fixture' and custom:raise SafeError('FIXTURE_CUSTOM_STATE_FORBIDDEN: canned responses must not evaluate real input')
    state=f['state'] if not custom else state
    state,found=screen(state)
    if found:raise SafeError('INPUT_DATA_BLOCKED: '+','.join(found))
    qs=questions_for(s,state)
    provider=None
    requested_model=MODEL
    if mode=='fixture':raw=simulated_response(qs,f['mock_values'],f['mock_uncertain']);response=validate_response(raw,qs)
    else:
        from . import client
    ...
    'fixture_contract_expected_status':f['expected_status'] if mode=='fixture' else None,
    'fixture_contract_passed':decision['status']==f['expected_status'] if mode=='fixture' else None,
