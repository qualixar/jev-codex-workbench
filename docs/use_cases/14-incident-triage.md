# 14-incident-triage — Triage a service incident

**Industry context:** DevOps and service operations. **Category:** Operations.

## What this demonstrates

Recommend an investigation queue from supplied telemetry; do not claim root cause or trigger remediation.
...
Required top-level state fields: `summary`, `observations`.
The executable source of truth is `use_cases/14-incident-triage.json`.
...
```bash
.venv/bin/python jev.py describe 14-incident-triage
.venv/bin/python jev.py run 14-incident-triage
.venv/bin/python jev.py run 14-incident-triage --variant adversarial
.venv/bin/python jev.py run 14-incident-triage --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 14-incident-triage --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 14-incident-triage --orientation both --seconds 8
```
