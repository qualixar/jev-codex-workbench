# 04-file-ranking — Rank repository file candidates

**Industry context:** Software engineering. **Category:** Context.

## What this demonstrates

Score supplied file summaries before Codex reads source; do not claim this replaces symbol search or proves dead code.
...
The executable source of truth is `use_cases/04-file-ranking.json`.
...
```bash
.venv/bin/python jev.py describe 04-file-ranking
.venv/bin/python jev.py run 04-file-ranking
.venv/bin/python jev.py run 04-file-ranking --variant adversarial
.venv/bin/python jev.py run 04-file-ranking --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 04-file-ranking --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 04-file-ranking --orientation both --seconds 8
```
