# 18-support-triage — Route a support request

**Industry context:** Customer support. **Category:** Industry.

## What this demonstrates

Classify a synthetic support message for a queue; no automated refunds or promises.
...
Required top-level state fields: `message`.
The executable source of truth is `use_cases/18-support-triage.json`.
...
```bash
.venv/bin/python jev.py describe 18-support-triage
.venv/bin/python jev.py run 18-support-triage
.venv/bin/python jev.py run 18-support-triage --variant adversarial
.venv/bin/python jev.py run 18-support-triage --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 18-support-triage --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 18-support-triage --orientation both --seconds 8
```
