# 07-claim-verification — Verify a claim against evidence

**Industry context:** Software engineering. **Category:** Verification.

## What this demonstrates

Check a claim against provided sources; a positive result is not verification beyond those sources.
...
Required top-level state fields: `claim`, `evidence`.
The executable source of truth is `use_cases/07-claim-verification.json`.
...
Policy kind: `gate`. Default thresholds are demonstration settings,
not evidence of calibration or suitability for a high-stakes workflow.

## Run without spending API calls

```bash
.venv/bin/python jev.py describe 07-claim-verification
.venv/bin/python jev.py run 07-claim-verification
.venv/bin/python jev.py run 07-claim-verification --variant adversarial
.venv/bin/python jev.py run 07-claim-verification --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 07-claim-verification --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 07-claim-verification --orientation both --seconds 8
```
