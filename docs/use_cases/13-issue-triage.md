# 13-issue-triage — Triage an engineering issue

**Industry context:** Software engineering. **Category:** Operations.

## What this demonstrates

Classify a synthetic issue for routing without modifying GitHub or Jira.
...
Required top-level state fields: `title`, `description`.
The executable source of truth is `use_cases/13-issue-triage.json`.
...
```bash
.venv/bin/python jev.py describe 13-issue-triage
.venv/bin/python jev.py run 13-issue-triage
.venv/bin/python jev.py run 13-issue-triage --variant adversarial
.venv/bin/python jev.py run 13-issue-triage --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 13-issue-triage --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 13-issue-triage --orientation both --seconds 8
```
