# 03-tool-selection — Select the next bounded tool

**Industry context:** Software engineering. **Category:** Routing.

## What this demonstrates

Choose a known tool; an unlisted tool must never be invented or executed.
...
Required top-level state fields: `goal`, `available_tools`.
The executable source of truth is `use_cases/03-tool-selection.json`.
...
```bash
.venv/bin/python jev.py describe 03-tool-selection
.venv/bin/python jev.py run 03-tool-selection
.venv/bin/python jev.py run 03-tool-selection --variant adversarial
.venv/bin/python jev.py run 03-tool-selection --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 03-tool-selection --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 03-tool-selection --orientation both --seconds 8
```
