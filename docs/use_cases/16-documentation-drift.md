# 16-documentation-drift — Detect documentation drift

**Industry context:** Software engineering. **Category:** Verification.

## What this demonstrates

Compare supplied documentation with implementation evidence and flag contradictions.
...
Required top-level state fields: `documentation`, `implementation`.
The executable source of truth is `use_cases/16-documentation-drift.json`.
...
```bash
.venv/bin/python jev.py describe 16-documentation-drift
.venv/bin/python jev.py run 16-documentation-drift
.venv/bin/python jev.py run 16-documentation-drift --variant adversarial
.venv/bin/python jev.py run 16-documentation-drift --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 16-documentation-drift --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 16-documentation-drift --orientation both --seconds 8
```
