# 19-research-ranking — Rank research evidence candidates

**Industry context:** Research operations. **Category:** Industry.

## What this demonstrates

Rank supplied synthetic abstracts for relevance; do not infer scientific truth or create citations.
...
Required top-level state fields: `query`, `items`.
The executable source of truth is `use_cases/19-research-ranking.json`.
...
Policy kind: `rank`. Default thresholds are demonstration settings,
not evidence of calibration or suitability for a high-stakes workflow.

## Run without spending API calls

```bash
.venv/bin/python jev.py describe 19-research-ranking
.venv/bin/python jev.py run 19-research-ranking
.venv/bin/python jev.py run 19-research-ranking --variant adversarial
.venv/bin/python jev.py run 19-research-ranking --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 19-research-ranking --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 19-research-ranking --orientation both --seconds 8
```
