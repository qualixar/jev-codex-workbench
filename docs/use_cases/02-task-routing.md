# 02-task-routing — Route work by reasoning need

**Industry context:** Software engineering. **Category:** Routing.

## What this demonstrates

Recommend deterministic tooling, current fast model, deeper reasoning, or human review without changing subscriptions or model configuration.
...
Required top-level state fields: `request`, `constraints`.
The executable source of truth is `use_cases/02-task-routing.json`.
...
```bash
.venv/bin/python jev.py describe 02-task-routing
.venv/bin/python jev.py run 02-task-routing
.venv/bin/python jev.py run 02-task-routing --variant adversarial
.venv/bin/python jev.py run 02-task-routing --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 02-task-routing --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 02-task-routing --orientation both --seconds 8
```
