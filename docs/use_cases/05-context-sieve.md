# 05-context-sieve — Filter retrieved context

**Industry context:** Software engineering. **Category:** Context.

## What this demonstrates

Recommend keep, flag conflict, review, or quarantine for supplied passages. This is not an injection-proof firewall.
...
The executable source of truth is `use_cases/05-context-sieve.json`.
...
Policy kind: `context`. Default thresholds are demonstration settings,
not evidence of calibration or suitability for a high-stakes workflow.

## Run without spending API calls

```bash
.venv/bin/python jev.py describe 05-context-sieve
.venv/bin/python jev.py run 05-context-sieve
.venv/bin/python jev.py run 05-context-sieve --variant adversarial
.venv/bin/python jev.py run 05-context-sieve --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 05-context-sieve --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 05-context-sieve --orientation both --seconds 8
```
