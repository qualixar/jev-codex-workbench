# 17-security-review-routing — Route security-sensitive changes

**Industry context:** Software engineering. **Category:** Safety.

## What this demonstrates

Identify review specialism without asserting vulnerability absence or granting authorization.
...
Required top-level state fields: `change`, `scope`.
The executable source of truth is `use_cases/17-security-review-routing.json`.
...
```bash
.venv/bin/python jev.py describe 17-security-review-routing
.venv/bin/python jev.py run 17-security-review-routing
.venv/bin/python jev.py run 17-security-review-routing --variant adversarial
.venv/bin/python jev.py run 17-security-review-routing --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 17-security-review-routing --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 17-security-review-routing --orientation both --seconds 8
```
