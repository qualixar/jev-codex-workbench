# 15-test-selection — Recommend relevant tests

**Industry context:** Software engineering. **Category:** Verification.

## What this demonstrates

Recommend additional focused tests; never remove mandatory test suites from the existing gate.
...
The executable source of truth is `use_cases/15-test-selection.json`.
...
Policy kind: `test_select`. Default thresholds are demonstration settings,
not evidence of calibration or suitability for a high-stakes workflow.

## Run without spending API calls

```bash
.venv/bin/python jev.py describe 15-test-selection
.venv/bin/python jev.py run 15-test-selection
.venv/bin/python jev.py run 15-test-selection --variant adversarial
.venv/bin/python jev.py run 15-test-selection --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 15-test-selection --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 15-test-selection --orientation both --seconds 8
```
