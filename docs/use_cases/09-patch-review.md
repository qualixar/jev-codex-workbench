# 09-patch-review — Review semantic patch risk

**Industry context:** Software engineering. **Category:** Verification.

## What this demonstrates

Identify revision or specialist-review needs after deterministic tests; never approve a production merge.
...
Required top-level state fields: `requirement`, `patch`.
The executable source of truth is `use_cases/09-patch-review.json`.
...
```bash
.venv/bin/python jev.py describe 09-patch-review
.venv/bin/python jev.py run 09-patch-review
.venv/bin/python jev.py run 09-patch-review --variant adversarial
.venv/bin/python jev.py run 09-patch-review --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 09-patch-review --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 09-patch-review --orientation both --seconds 8
```
