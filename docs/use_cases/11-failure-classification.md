# 11-failure-classification — Classify bounded-loop failures

**Industry context:** Software engineering. **Category:** Agent loops.

## What this demonstrates

Return a recovery recommendation using CTX, TOOL, PLAN, CAP, GATE, STALL, POLICY, BUDGET, or UNKNOWN.
...
Required top-level state fields: `goal`, `observations`.
The executable source of truth is `use_cases/11-failure-classification.json`.
...
```bash
.venv/bin/python jev.py describe 11-failure-classification
.venv/bin/python jev.py run 11-failure-classification
.venv/bin/python jev.py run 11-failure-classification --variant adversarial
.venv/bin/python jev.py run 11-failure-classification --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 11-failure-classification --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 11-failure-classification --orientation both --seconds 8
```
