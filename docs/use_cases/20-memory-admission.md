# 20-memory-admission — Review a memory admission proposal

**Industry context:** Software engineering. **Category:** Agent loops.

## What this demonstrates

Recommend retain, skip, conflict review, or human review. No SuperLocalMemory or SLM-Mesh write occurs.
...
Required top-level state fields: `candidate`, `existing`, `retention_policy`.
The executable source of truth is `use_cases/20-memory-admission.json`.
...
```bash
.venv/bin/python jev.py describe 20-memory-admission
.venv/bin/python jev.py run 20-memory-admission
.venv/bin/python jev.py run 20-memory-admission --variant adversarial
.venv/bin/python jev.py run 20-memory-admission --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 20-memory-admission --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 20-memory-admission --orientation both --seconds 8
```
