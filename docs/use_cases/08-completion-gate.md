# 08-completion-gate — Check a coding completion claim

**Industry context:** Software engineering. **Category:** Verification.

## What this demonstrates

Combine semantic checks with deterministic checks of supplied test receipts; do not trust the agent saying done.
...
Required top-level state fields: `requirement`, `diff_summary`, `evidence`.
The executable source of truth is `use_cases/08-completion-gate.json`.
...
Policy kind: `completion`. Default thresholds are demonstration settings,
not evidence of calibration or suitability for a high-stakes workflow.

## Run without spending API calls

```bash
.venv/bin/python jev.py describe 08-completion-gate
.venv/bin/python jev.py run 08-completion-gate
.venv/bin/python jev.py run 08-completion-gate --variant adversarial
.venv/bin/python jev.py run 08-completion-gate --variant uncertain
```

| Scenario | Expected local status | Interpretation |
|---|---|---|
| nominal | `CHECKS_PASSED` | Local fixture policy expectation only |
...
```bash
.venv/bin/python jev.py run 08-completion-gate --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 08-completion-gate --orientation both --seconds 8
```
