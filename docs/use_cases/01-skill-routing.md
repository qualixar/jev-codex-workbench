# 01-skill-routing — Select a relevant Codex skill

**Industry context:** Software engineering. **Category:** Routing.

## What this demonstrates

Choose one relevant skill or explicitly choose none; do not load the whole catalogue.
...
Required top-level state fields: `request`, `skills`.
The executable source of truth is `use_cases/01-skill-routing.json`.
...
```bash
.venv/bin/python jev.py describe 01-skill-routing
.venv/bin/python jev.py run 01-skill-routing
.venv/bin/python jev.py run 01-skill-routing --variant adversarial
.venv/bin/python jev.py run 01-skill-routing --variant uncertain
```
...
```bash
.venv/bin/python scripts/capture.py --case 01-skill-routing --orientation both --seconds 8
```
