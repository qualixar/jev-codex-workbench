# 06-injection-triage — Triage suspicious instructions

**Industry context:** Software engineering. **Category:** Safety.

## What this demonstrates

Flag text that tries to redirect the agent, while retaining deterministic trust boundaries and human review.
...
Required top-level state fields: `source_kind`, `text`.
The executable source of truth is `use_cases/06-injection-triage.json`.
...
```bash
.venv/bin/python jev.py describe 06-injection-triage
.venv/bin/python jev.py run 06-injection-triage
.venv/bin/python jev.py run 06-injection-triage --variant adversarial
.venv/bin/python jev.py run 06-injection-triage --variant uncertain
```
...
```bash
.venv/bin/python jev.py run 06-injection-triage --mode live --scope project-live --workspace "$PWD"
```
...
```bash
.venv/bin/python scripts/capture.py --case 06-injection-triage --orientation both --seconds 8
```
