...
class Engine:
    ...
    def judge(self,recipe,state,questions,p=None):
        p=p or self.policy();validate_policy(p,self.workspace)
        if digest(p)!=digest(self.policy()):raise AutoError('POLICY_CHANGED')
        if recipe not in p.get('case_ids',[]) and recipe not in p['local_recipe_ids']:raise AutoError('RECIPE_NOT_ENROLLED')
        ...
        request={'state':state,'questions':questions,'provider':effective['provider'],'model':identity,'recipe':recipe,'policy':digest(p),'runtime':'1.1.3'}
        ...
        def work():
            ...
            result=validate_response(result,questions)
    ...
    def dispatch(self,req):
        ...
        if op=='health':
            ...
            return {'version':'1.1.3','active':active,'provider':provider,'native_browser_verified':False,'actual_host_savings_measured':False}
