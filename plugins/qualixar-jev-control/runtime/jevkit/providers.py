...
def default_config_root() -> Path:
    ...
    if base:
        return Path(base).expanduser() / "qualixar-jev-control"
    return Path.home() / ".config" / "qualixar-jev-control"
...
def resolve_provider(
    ...
    if not selected:
        ...
        legacy_typesafe = Path.home() / ".config" / "jev-codex-workbench" / "api-key"
        if len(available) == 1:
            selected = available[0]
        elif not available and legacy_typesafe.exists():
            selected = "typesafe"
        elif not available:
            selected = "typesafe"
        else:
...
def get_provider_credential(
    ...
    if not path.exists() and profile.provider_id == "typesafe" and config_root is None:
        legacy = Path.home() / ".config" / "jev-codex-workbench" / "api-key"
        if legacy.exists():
            path = legacy
    if not path.exists():
        raise SafeError(
